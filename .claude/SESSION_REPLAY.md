...
```
PRD 文件：prd/[功能名称]_[日期].md
需求分析：prd/需求分析/[功能名称]_[日期].md
...
我们还没做推送 GitHub 这步。如果需要推送：

```bash
cd ~/Desktop/PM-AI-Context
git init
git add .
git commit -m "初始化 PM AI 工作台"
# 然后创建 GitHub 仓库并关联
```
...
1. **推送 GitHub** — 把整个项目推上去，以后换电脑也能用
2. **补充 context** — 把更多产品背景写进 context/
